...
const PLANS = [
  ...
  },
  {
    ...
    buttonText: 'Trải nghiệm tối đa',
    buttonClass: 'btn-primary bg-gradient-to-r from-amber-500 to-orange-600 border-none',
  },
];
          ...
          {PLANS.map((plan, index) => (
            <motion.div
              ...
              className={`relative glass-card flex flex-col h-full border-2 ${
                plan.popular ? 'border-blue-500/40' : 'border-[var(--color-border)]'
              }`}
            >
              {plan.popular && (
              ...
              <button 
                className={`w-full py-4 px-6 rounded-xl font-bold transition-all duration-200 flex items-center justify-center gap-2 group ${
                  plan.isCurrent ? 'bg-slate-800/50 text-slate-400 cursor-not-allowed' : plan.buttonClass
                }`}
                disabled={plan.isCurrent}
              >
                {plan.buttonText}
                {!plan.isCurrent && <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />}
              </button>
            </motion.div>
          ))}
        </div>
